...
public class Send_Mail {
    ...
    public static int sendMail(String tomail) {
        int reval = 0;
        boolean b = false;
        String uid = "";
        ...
        if (b) {
            String link ="";
            try {
               link += "http://localhost:8080/Projectssale.com/New_Password.jsp?id=" + uid + "&val=" + Security.Security.encrypt(val);
            } catch (Exception e) {
            }
            ...
            message += "Please Go to the link";
            message += "<a href='" + link + "'><button style=' width='40px' height='200px' '>Recovery My Password</button></a>";
